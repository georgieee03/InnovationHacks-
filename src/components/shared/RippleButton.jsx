...
export default function RippleButton({
  ...
  icon: Icon, className = '', disabled = false, ...props
}) {
  ...
  return (
    <motion.button
      ...
      className={`relative overflow-hidden inline-flex items-center justify-center gap-2 font-semibold rounded-lg border backdrop-blur-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${sizes[size]} ${className}`}
    ...
    >
      ...
      <AnimatePresence>
        {ripples.map((ripple) => (
          <motion.span key={ripple.id} className="absolute rounded-full bg-white/30 pointer-events-none"
