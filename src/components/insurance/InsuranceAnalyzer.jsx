...
export default function InsuranceAnalyzer() {
  const { businessInfo, riskFactors, policySummary, setPolicySummary, gapAnalysis, setGapAnalysis } = useContext(AppContext);
  ...
  const handleFileSelect = useCallback(async (file) => {
    ...
    try {
      ...
      setError('Analysis failed. Try again or use the demo policy.');
      setIsComplete(false);
    } finally {
  ...
  const handleLoadDemo = useCallback(async () => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error('Failed to fetch demo PDF');
      const blob = await resp.blob();
      const file = new File([blob], 'marias-bakery-policy.pdf', { type: 'application/pdf' });
      await handleFileSelect(file);
    } catch (err) {
      console.error('Demo load failed:', err);
      setError('Could not load demo policy.');
      setIsAnalyzing(false);
    }
  }, [handleFileSelect]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {!isComplete && !isAnalyzing && (
            <button
              ...
              className="relative z-10 px-4 py-2 text-sm font-normal bg-primary text-white rounded-lg hover:bg-primary/90 transition whitespace-nowrap"
            >
              Load Demo Policy
            </button>
          )}
      ...
      <AnimatePresence>
        {policySummary && (
          <motion.div key="policy-summary" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5, delay: 0.2 }}>
      ...
      <AnimatePresence>
        {gapAnalysis && (
          <motion.div key="gap-analysis" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5, delay: 0.4 }}>
