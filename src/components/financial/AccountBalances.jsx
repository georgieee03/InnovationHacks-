...
const cardVariants = {
  ...
  visible: (i) => ({ opacity: 1, x: 0, transition: { duration: 0.4, delay: i * 0.1 } }),
};
...
function AccountCard({ acct, index }) {
  ...
  return (
    <motion.div ref={cardRef}
      ...
      custom={index} initial="hidden" animate="visible" variants={cardVariants}
  ...
  return (
    <div className="glass-card p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {accounts.map((acct, i) => <AccountCard key={acct.id} acct={acct} index={i} />)}
      </div>
    </div>
  );
}