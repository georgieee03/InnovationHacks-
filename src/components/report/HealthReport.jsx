...
function computeHealthScore(financialMetrics, gapAnalysis) {
  ...
  if (gapItems.length > 0) {
    ...
    const covered = gapItems.filter((g) => g.status === 'covered').length;
...
function getStrengths(financialMetrics, gapAnalysis) {
  ...
  if (financialMetrics) {
    ...
    if (financialMetrics.averageMonthlyIncome > financialMetrics.averageMonthlyExpenses) {
      strengths.push('Average income exceeds average expenses');
    }
  }
...
function getRisks(financialMetrics, gapAnalysis, riskFactors) {
  ...
  if (financialMetrics) {
    if (financialMetrics.monthsOfRunway < 3) {
      risks.push(`Only ${financialMetrics.monthsOfRunway} months of runway, below the 3-month minimum.`);
    }

    if (financialMetrics.emergencyFundGap > 0) {
      risks.push(`Emergency fund gap of ${formatCurrency(financialMetrics.emergencyFundGap)}`);
    }
  ...
  if (gapItems.length > 0) {
    const uncovered = gapItems.filter((g) => g.status === 'gap');
    uncovered.forEach((gap) => risks.push(`No coverage for ${gap.type || gap.category}`));
  }

  if (riskFactors) {
    if (riskFactors.floodZone || riskMap.flood_zone) risks.push('Located in a flood zone without flood coverage');
    if (riskFactors.crimeRate === 'high' || riskMap.crime?.level === 'high') {
      risks.push('High crime rate area, consider additional security measures.');
    }
  }
...
function getRecommendations(financialMetrics, gapAnalysis) {
  ...
  if (financialMetrics && financialMetrics.emergencyFundGap > 0) {
    recommendations.push({
      action: 'Build emergency fund',
      cost: formatCurrency(financialMetrics.emergencyFundGap),
      impact: 'High',
      detail: 'Save monthly to close the gap within 12 months',
    });
  }
  ...
  if (gapItems.length > 0) {
    ...
    if (uncovered.length > 0) {
      recommendations.push({
        action: `Get ${uncovered[0].type || 'missing'} coverage`,
        cost: '$500-$2,000/yr',
        impact: 'High',
        detail: 'Eliminates the largest coverage gap',
      });
    }

    const underinsured = gapItems.filter((g) => g.status === 'underinsured');
    if (underinsured.length > 0) {
      recommendations.push({
        action: `Increase ${underinsured[0].type || 'underinsured'} limits`,
        cost: '$200-$800/yr',
        impact: 'Medium',
        detail: 'Brings coverage to recommended levels',
      });
    }
  }
  ...
  if (recommendations.length < 3) {
    recommendations.push({
      action: 'Schedule annual policy review',
      cost: 'Free',
      impact: 'Medium',
      detail: 'Ensure coverage keeps pace with business growth',
    });
  }
...
export default function HealthReport() {
  ...
  return (
    <div className="health-report">
        ...
        <div className="flex gap-3">
          ...
          {generated && (
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-gray-200 px-5 py-2.5 text-sm font-medium text-text-primary transition hover:bg-gray-50"
            >
      ...
      {!generated ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl p-12 text-center shadow-md">
        ...
        <div className="print-report space-y-6">
          <motion.div {...sectionAnim} className="bg-card rounded-xl p-6 shadow-md">
          ...
          <motion.div {...sectionAnim} transition={{ delay: 0.1 }} className="bg-card rounded-xl p-6 shadow-md">
          ...
          <motion.div {...sectionAnim} transition={{ delay: 0.2 }} className="bg-card rounded-xl p-6 shadow-md">
            <div className="mb-4 flex items-center gap-2">
          ...
          <motion.div {...sectionAnim} transition={{ delay: 0.3 }} className="bg-card rounded-xl p-6 shadow-md">
            <div className="mb-4 flex items-center gap-2">
              ...
              {recommendations.map((recommendation, index) => (
                <div key={index} className="flex items-start gap-4 rounded-lg bg-gray-50 p-3">
                  ...
                  <div className="flex-shrink-0 text-right">
                    ...
                    <p className={`text-xs font-medium ${recommendation.impact === 'High' ? 'text-gap' : 'text-underinsured'}`}>
          ...
          {financialMetrics?.monthlyBreakdown && (
            <motion.div {...sectionAnim} transition={{ delay: 0.4 }} className="bg-card rounded-xl p-6 shadow-md">
                  ...
                  <tbody>
                    {financialMetrics.monthlyBreakdown.map((month) => (
                      <tr key={month.month} className="border-b border-gray-100">
                        ...
                        <td className={`py-2 text-right font-medium ${month.netCashFlow >= 0 ? 'text-covered' : 'text-gap'}`}>
