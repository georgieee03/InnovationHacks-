/**
 * DocumentsWorkspace — Contracts, Quotes, Receipts, Compliance in one place.
 * Each tab loads its own data lazily on first activation — no upfront waterfall.
 */
...
export default function DocumentsWorkspace() {
  ...
  // Per-tab data — loaded lazily on first visit
    ...
    <div className="glass-card rounded-[24px] p-5">
      ...
      {analysis.missingProtections?.length > 0 && (
        <div className="mt-3 rounded-xl border border-warning/20 bg-warning/5 p-3">
          ...
          {analysis.missingProtections.map((p, i) => <p key={i} className="text-xs text-warning/80 leading-relaxed">• {p}</p>)}
        </div>
      )}
    </div>
  );
