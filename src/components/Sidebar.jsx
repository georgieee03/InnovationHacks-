      ...
      {isOnboarded && businessInfo && (
        <>
          <div className="border-b border-white/10 px-5 py-5">
            ...
            {typeInfo && (
              <p className="mt-1 text-sm font-light text-gray-300">{typeInfo.icon} {typeInfo.label}</p>
            )}
            ...
            <div className="px-5 py-5">
              ...
              <div className="flex flex-col gap-2">
                {Object.entries(riskFactors.risks).map(([key, risk]) => (
                  <div
                    key={key}
                    className="flex items-center gap-2 rounded-lg border border-white/5 bg-sidebar-hover px-3 py-2 transition-all duration-200 hover:border-white/10"
                  >
                    <span
